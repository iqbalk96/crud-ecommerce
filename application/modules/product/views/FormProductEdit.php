...
<body>
    <?php $this->load->view('partials/TheSidebar.php'); ?>
    <main class="content flex-fill">
        <section>
            ...
            <nav class="nav-content gap-5">
                <div class="d-flex gap-3 align-items-center">
                    <img src="<?php echo base_url('assets/backend/assets/') ?>assets/images/photo.webp" alt="Photo Profile" class="photo-profile" />
                    <div>
                        <p class="title-content mb-2">Good Morning, <?= $this->session->userdata('fullname'); ?></p>
                ...
                <div class="search-wrapper">
                    <div class="search-bar flex-fill">
                        <input class="form-control" type="text" placeholder="Search shoes" />
                    </div>
                    <button class="btn-setting rounded-circle" aria-label="Button Setting">
                        <svg fill="none">
        ...
        <section class="d-flex flex-column gap-4">
            <div class="d-flex justify-content-between align-items-center gap-3">
                <h4 class="title-section-content">Update Product</h4>
            </div>
            <div class="card">
                <div class="card-body">
                    <div>

                        <form enctype="multipart/form-data" action="<?php echo base_url('product/update_product/') ?><?= $product->id_barang ?>" method="post">
                            <div class="row">
                                <div class="col-12 mb-3">
                                    <label for="product_name" class="form-label">Product name</label>
                                    <input type="text" required class="form-control" name="nama" id="product_name" placeholder="eg: Louis Vouton" value="<?= $product->nama ?>">
                                </div>
                                <div class="col-6 mb-3">
                                    <label for="harga_beli" class="form-label">Purchase price</label>
                                    <input type="number" min="0" required class="form-control" name="harga_beli" id="harga_beli" placeholder="eg: 150000" value="<?= $product->harga_beli ?>">
                                </div>
                                <div class="col-6 mb-3">
                                    <label for="harga_jual" class="form-label">Selling price</label>
                                    <input type="number" min="0" required class="form-control" name="harga_jual" id="harga_jual" placeholder="eg: 200000" value="<?= $product->harga_jual ?>">
                                </div>
                                <div class="col-6 mb-3">
                                    <label for="unit" class="form-label">Unit</label>
                                    <select name="satuan" id="unit" required class="form-control">
                                        <option>[ Select unit ]</option>
                                        <option value="pcs" <?= $product->satuan == 'pcs' ? 'selected' : '' ?>>Pcs</option>
                                        <option value="lusin" <?= $product->satuan == 'lusin' ? 'selected' : '' ?>>Lusin</option>
                                    </select>
                                </div>
                                <div class="col-6 mb-3">
                                    <label for="category" class="form-label">Category</label>
                                    <select name="kategori" id="category" required class="form-control">
                                        <option>[ Select category ]</option>
                                        <option value="shoes" <?= $product->kategori == 'shoes' ? 'selected' : '' ?>>Shoes</option>
                                        <option value="bag" <?= $product->kategori == 'bag' ? 'selected' : '' ?>>Bag</option>
                                    </select>
                                </div>
                                <div class="col-12 mb-3">
    ...
    <?php $this->load->view('partials/TheScript.php'); ?>
</body>
